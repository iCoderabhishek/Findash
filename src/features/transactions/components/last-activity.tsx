...
import {
  ...
  ArrowLeft01Icon, ArrowRight01Icon, Download01Icon,
    ...
    <div className={cn(bentoCardStatic, 'h-full')}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-2">
          <h3 className="text-foreground font-light text-xl tracking-tight">Last <span className="font-bold">activity</span></h3>
          {selectedCategory && (
        ...
        <form onSubmit={handleSubmit} className="mb-6 p-4 bg-muted/40 border border-border/40 rounded-2xl animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 items-end">
            ...
            <div>
              ...
              <input type="number" required step="10" min="1" value={formData.amount} onChange={e => setFormData({ ...formData, amount: e.target.value })}
        ...
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent border-border/40">
              ...
              {role === 'Admin' && <TableHead className="text-[13px] font-medium text-muted-foreground text-right">Actions</TableHead>}
            </TableRow>
          </TableHeader>
          <TableBody>
            {paginatedData.length > 0 ? (
              paginatedData.map((tx) => (
                <TableRow
                  key={tx.id}
                  className="group relative border-border/30"
                ...
                >
                  ...
                  <TableCell className={cn('text-sm text-right font-bold tabular-nums relative z-10', tx.type === 'Income' ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400')}>
                    {tx.type === 'Income' ? '+' : '-'}₹{tx.amount.toFixed(2)}
                  </TableCell>
                  {role === 'Admin' && (
                    <TableCell className="text-right relative z-10">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                        <Button variant="ghost" size="icon-xs" className="rounded-full" onClick={() => openForm(tx)}>
                          <HugeiconsIcon icon={PencilEdit01Icon} size={24} />
                        </Button>
                        <Button variant="ghost" size="icon-xs" className="rounded-full" onClick={() => deleteTransaction(tx.id)}>
                          <HugeiconsIcon icon={Delete01Icon} size={24} className="text-rose-500" />
                        </Button>
                      </div>
                    </TableCell>
                  )}
